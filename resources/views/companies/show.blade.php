        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 flex items-center space-x-6">
                    @if($company->logo)
                        <img src="{{ $company->logo->path }}" alt="{{ $company->name }}" class="w-24 h-24 object-cover rounded-lg">
                    @endif
                    <div>
                            ...
                            <div class="space-y-4">
                                @foreach($company->founders as $founder)
                                    <div class="flex items-center space-x-3">
                                        @if($founder->avatar)
                                            <img src="{{ $founder->avatar }}" alt="{{ $founder->name }}" class="w-10 h-10 rounded-full">
                                        @endif
                                        <div>
                        ...
                        <div class="border rounded-lg p-4">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                @foreach($company->investors as $investor)
                                    <div class="flex items-center space-x-3">
                                        @if($investor->logo)
                                            <img src="{{ $investor->logo->path }}" alt="{{ $investor->name }}" class="w-8 h-8 object-cover rounded">
                                        @endif
                                ...
                                @foreach($company->alternatives as $alternative)
                                    <div>
                                        ...
                                        @if ($alternative->url && $alternative->url !== '#')
                                            <a href="{{ $alternative->url }}" class="text-blue-500 hover:underline">{{ $alternative->url }}</a>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="p-6 border-t">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            @foreach($company->resources as $resource)
                                <div class="border rounded-lg p-4">
                                    <a href="{{ $resource->url }}" class="font-medium text-blue-500 hover:underline">{{ $resource->url }}</a>
                                </div>
                            @endforeach
                        </div>
                    </div>
